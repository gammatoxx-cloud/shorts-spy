import { createClient } from "@/lib/supabase/server";
import { Database } from "@/types/database.types";

type TikTokVideo = Database["public"]["Tables"]["tiktok_videos"]["Row"];
type TikTokVideoInsert = Database["public"]["Tables"]["tiktok_videos"]["Insert"];

/**
 * Calculate engagement rate: (likes + comments) / views * 100
 */
function calculateEngagementRate(
  likes: number,
  comments: number,
  views: number
): number {
  if (views === 0) return 0;
  return Number(((likes + comments) / views) * 100).toFixed(4);
}

/**
 * Insert or update a video (upsert by video_id)
 */
export async function upsertVideo(
  video: TikTokVideoInsert
): Promise<TikTokVideo> {
  const supabase = await createClient();

  // Calculate engagement rate if not provided
  const engagementRate =
    video.engagement_rate ??
    calculateEngagementRate(
      video.likes || 0,
      video.comments || 0,
      video.views || 0
    );

  const videoData: TikTokVideoInsert = {
    ...video,
    engagement_rate: engagementRate,
  };

  const { data, error } = await supabase
    .from("tiktok_videos")
    .upsert(videoData, {
      onConflict: "video_id",
      ignoreDuplicates: false,
    })
    .select()
    .single();

  if (error) throw error;
  return data;
}

/**
 * Insert multiple videos in a batch
 */
export async function insertVideos(
  videos: TikTokVideoInsert[]
): Promise<TikTokVideo[]> {
  const supabase = await createClient();

  // Calculate engagement rates for all videos
  const videosWithEngagement = videos.map((video) => ({
    ...video,
    engagement_rate:
      video.engagement_rate ??
      calculateEngagementRate(
        video.likes || 0,
        video.comments || 0,
        video.views || 0
      ),
  }));

  const { data, error } = await supabase
    .from("tiktok_videos")
    .upsert(videosWithEngagement, {
      onConflict: "video_id",
      ignoreDuplicates: false,
    })
    .select();

  if (error) throw error;
  return data || [];
}

/**
 * Get videos for a profile
 */
export async function getProfileVideos(
  profileId: string,
  options?: {
    limit?: number;
    offset?: number;
    orderBy?: "engagement_rate" | "views" | "likes" | "comments" | "posted_at";
    orderDirection?: "asc" | "desc";
  }
): Promise<TikTokVideo[]> {
  const supabase = await createClient();

  let query = supabase
    .from("tiktok_videos")
    .select("*")
    .eq("profile_id", profileId);

  // Apply ordering
  const orderBy = options?.orderBy || "engagement_rate";
  const orderDirection = options?.orderDirection || "desc";
  query = query.order(orderBy, { ascending: orderDirection === "asc" });

  // Apply pagination
  if (options?.limit) {
    query = query.limit(options.limit);
  }
  if (options?.offset) {
    query = query.range(options.offset, options.offset + (options.limit || 100) - 1);
  }

  const { data, error } = await query;

  if (error) throw error;
  return data || [];
}

/**
 * Get videos for a specific scrape
 */
export async function getScrapeVideos(scrapeId: string): Promise<TikTokVideo[]> {
  const supabase = await createClient();
  const { data, error } = await supabase
    .from("tiktok_videos")
    .select("*")
    .eq("scrape_id", scrapeId)
    .order("engagement_rate", { ascending: false });

  if (error) throw error;
  return data || [];
}

/**
 * Get video statistics for a profile
 */
export async function getProfileVideoStats(profileId: string): Promise<{
  totalVideos: number;
  totalViews: number;
  totalLikes: number;
  totalComments: number;
  averageViews: number;
  averageLikes: number;
  averageEngagementRate: number;
  bestVideo: TikTokVideo | null;
  postingFrequency: {
    perWeek: number;
    perMonth: number;
  };
}> {
  const supabase = await createClient();

  const { data: videos, error } = await supabase
    .from("tiktok_videos")
    .select("*")
    .eq("profile_id", profileId);

  if (error) throw error;

  if (!videos || videos.length === 0) {
    return {
      totalVideos: 0,
      totalViews: 0,
      totalLikes: 0,
      totalComments: 0,
      averageViews: 0,
      averageLikes: 0,
      averageEngagementRate: 0,
      bestVideo: null,
      postingFrequency: {
        perWeek: 0,
        perMonth: 0,
      },
    };
  }

  const totalVideos = videos.length;
  const totalViews = videos.reduce((sum, v) => sum + v.views, 0);
  const totalLikes = videos.reduce((sum, v) => sum + v.likes, 0);
  const totalComments = videos.reduce((sum, v) => sum + v.comments, 0);
  const averageViews = totalVideos > 0 ? totalViews / totalVideos : 0;
  const averageLikes = totalVideos > 0 ? totalLikes / totalVideos : 0;
  const averageEngagementRate =
    videos.reduce((sum, v) => sum + Number(v.engagement_rate), 0) / totalVideos;

  const bestVideo =
    videos.reduce((best, current) =>
      Number(current.engagement_rate) > Number(best.engagement_rate)
        ? current
        : best
    ) || null;

  // Calculate posting frequency
  const videosWithDates = videos.filter((v) => v.posted_at);
  let postingFrequency = { perWeek: 0, perMonth: 0 };

  if (videosWithDates.length > 1) {
    // Sort by date
    const sortedDates = videosWithDates
      .map((v) => new Date(v.posted_at!).getTime())
      .sort((a, b) => a - b);

    const oldestDate = sortedDates[0];
    const newestDate = sortedDates[sortedDates.length - 1];
    const daysDiff = (newestDate - oldestDate) / (1000 * 60 * 60 * 24);

    if (daysDiff > 0) {
      postingFrequency.perWeek = (videosWithDates.length / daysDiff) * 7;
      postingFrequency.perMonth = (videosWithDates.length / daysDiff) * 30;
    }
  }

  return {
    totalVideos,
    totalViews,
    totalLikes,
    totalComments,
    averageViews: Math.round(averageViews),
    averageLikes: Math.round(averageLikes),
    averageEngagementRate: Number(averageEngagementRate.toFixed(4)),
    bestVideo,
    postingFrequency: {
      perWeek: Number(postingFrequency.perWeek.toFixed(1)),
      perMonth: Number(postingFrequency.perMonth.toFixed(1)),
    },
  };
}

