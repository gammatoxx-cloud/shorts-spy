...
export default function YoutubeResultsPage() {
  ...
  const fetchData = async () => {
    ...
    try {
      const response = await fetch(`/api/youtube/creators/${handle}`);
      if (!response.ok) {
  ...
  return (
    <div className="container mx-auto px-4 py-8 relative z-10">
      <div className="max-w-6xl mx-auto">
              ...
              {profile.avatar_url ? (
                <div className="relative">
                  <img
                    ...
                    className="w-24 h-24 rounded-full object-cover border-2 border-red-500/50 shadow-glow"
                  />
                  <div className="absolute inset-0 rounded-full border-2 border-red-500/20 animate-pulse"></div>
                </div>
              ) : (
                <div className="w-24 h-24 rounded-full bg-gradient-to-r from-red-500 to-orange-600 flex items-center justify-center text-white text-3xl font-bold border-2 border-red-500/50 shadow-glow">
                ...
                <div className="flex items-center gap-4">
                  {profile.follower_count && (
                    <div className="flex items-center gap-2 text-white/70">
                      ...
                      <span className="font-medium">{profile.follower_count.toLocaleString()} subscribers</span>
                    </div>
                  )}
            ...
            <div className="flex items-start gap-3">
              ...
              <div className="flex-1">
                <p className="text-sm text-yellow-300">
                  ...
                  {stats.totalVideos} shorts.{" "}
                  {videoLimit === 20 && (
                    <Link
                      href="/pricing"
                      className="font-medium hover:underline underline-offset-2 text-yellow-200"
                    >
                      Upgrade to paid to see all {stats.totalVideos} shorts
                    </Link>
                  )}
                </p>
              </div>
        ...
        <div className="mb-10" aria-label="Shorts">
          <div className="flex items-center justify-between mb-6">
            <h2 className="text-2xl font-bold text-white">Shorts</h2>
            <div className="text-sm text-white/50">
              {videos.length} {videos.length === 1 ? 'short' : 'shorts'}
            </div>
          </div>
          <VideoTable videos={videos} platform="youtube" />
        </div>
      </div>
    </div>
  );
