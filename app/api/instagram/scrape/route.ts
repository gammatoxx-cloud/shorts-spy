...
import { startInstagramScrape } from "@/lib/apify/instagram-scraper";
import {
 ...
 * Start a new Instagram Reels scraping job
 */
export async function POST(request: NextRequest) {
  try {
      ...
      return NextResponse.json(
        {
          ...
          message: `You can analyze up to ${userVideoLimit} reels. ${userVideoLimit === 20 ? "Upgrade to paid to analyze up to 100 reels." : ""}`
        },
        { status: 403 }
      );
    }
    ...
    const existingProfile = await getProfileByUsername(normalizedUsername, "instagram");
    if (existingProfile?.last_scraped_at) {
      ...
      if (hoursSinceScrape < 48) {
        ...
        const { getProfileReels, getProfileReelStats } = await import(
          "@/lib/db"
        );
        const reels = await getProfileReels(existingProfile.id, {
          limit: requestedVideoCount,
          orderBy: "engagement_rate",
          orderDirection: "desc",
        });

        // Only return cached data if we have reels
        // If no reels, proceed to scrape fresh data
        if (reels && reels.length > 0) {
          const stats = await getProfileReelStats(existingProfile.id);
        ...
        // If no reels in cache, continue to scrape
      }
    }
    ...
    try {
      console.log(`Starting Instagram scrape for user: ${normalizedUsername}, video count: ${requestedVideoCount}`);
      const run = await startInstagramScrape(normalizedUsername, requestedVideoCount);
      apifyRunId = run.id;
      console.log(`Instagram Apify run started successfully. Run ID: ${apifyRunId}, Status: ${run.status}`);
      ...
      console.error("Error starting Instagram Apify run:", error);
