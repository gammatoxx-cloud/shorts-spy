...
export default function AnalyzePage() {
  ...
  const handleSearch = async (username: string, videoCount: number) => {
    setIsSearching(true);
    setSearchError(null);
    setProgressMessage(selectedPlatform === "tiktok" ? "Starting analysis..." : "Initializing analysis...");
    ...
    try {
      ...
      if (error.message?.includes("Rate limit")) {
        ...
        errorMessage = `Please sign in to analyze ${selectedPlatform === "tiktok" ? "TikTok" : "Instagram"} creators.`;
      } else if (error.message?.includes("not found") || error.message?.includes("404")) {
        errorMessage = `${selectedPlatform === "tiktok" ? "TikTok" : "Instagram"} username not found. Please check the username and try again.`;
      ...
      if (selectedPlatform === "tiktok") {
        ...
        // Reset searching state after a delay for Instagram
        setTimeout(() => {
          setIsSearching(false);
        }, 2000);
      }
      ...
      <div className="max-w-4xl mx-auto">
        ...
        <div className="mb-12">
          <div className="flex items-center gap-3 mb-3">
            <div className={`w-12 h-12 rounded-xl bg-gradient-to-br ${selectedPlatform === "tiktok" ? "from-blue-500/20 to-purple-500/20 border border-blue-500/30" : "from-purple-500/20 to-pink-500/20 border border-purple-500/30"} flex items-center justify-center backdrop-blur-sm`}>
        ...
        <div className={`card-modern-search relative overflow-hidden mb-10 transition-all duration-300 ${isSearching ? 'opacity-60 pointer-events-none' : ''}`}>
          ...
          <div className={`absolute inset-0 bg-gradient-to-r ${selectedPlatform === "tiktok" ? "from-blue-500/10 via-purple-500/10 to-blue-500/10" : "from-purple-500/10 via-pink-500/10 to-purple-500/10"} opacity-50 pointer-events-none`}></div>
          <div className={`absolute top-0 left-0 right-0 h-[1px] bg-gradient-to-r from-transparent ${selectedPlatform === "tiktok" ? "via-blue-500/50" : "via-purple-500/50"} to-transparent`}></div>
          
          <div className="relative p-8">
            <div className="flex items-center gap-3 mb-6">
              <div className={`w-10 h-10 rounded-lg bg-gradient-to-br ${selectedPlatform === "tiktok" ? "from-blue-500/20 to-purple-500/20 border border-blue-500/30" : "from-purple-500/20 to-pink-500/20 border border-purple-500/30"} flex items-center justify-center`}>
                {selectedPlatform === "tiktok" ? (
                  <svg
